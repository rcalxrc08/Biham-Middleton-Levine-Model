...
//costruttore da file;
    ...
    {   file_X.open(filename_X);
        ...
        string line2;
        unsigned int cc=0;
        while(cc<r+3){getline(file_X, line);
        if(line.size()>0){line2=line;}
        cc++;}//l'ultima riga è la mia prima.
        i=0;
        while(i<line2.size()){
                        v[contatore]=line2[i];
                        i+=2;
...
unsigned int ii=0;
unsigned int UB;
while(ii+1<rank*tre){getline(file_X, line);ii++;}
if (rank==size-1){UB=r+1;}
else {if(rank==0){UB=(rank+1)*tre;}
	else{if(rank<size-1){UB=(rank+1)*tre;}
		else{UB=rig;	}}}
while(ii<UB){
             getline(file_X, line);
             unsigned int i1=0;
...
int ok[cols];
int ok1[cols];
for(unsigned int H=0;H<cols;H++){ok[H]=-1;ok1[H]=-1;} 
unsigned jj=0;
unsigned jj1=0;
      ...
      for(unsigned int k=0;k<rig-1;++k){
                ...
                if(v[k*cols+j]==bc && v[(k+1)*cols+j]==cv){
                 if(!T){T=true;}
                   ...
                   v[k*cols+j]=cv;
                   v[(k+1)*cols+j]=bc;
		   if(k==rig-2){ok[jj]=j;jj++;}//ok verrà inviato al successivo
		   if((k==0)&&(size!=1)){ok1[jj1]=j;jj1++;}//ok1 al precedente
                   ++k;
...
int buf[cols];
int buf1[cols];
for(unsigned int H=0;H<cols;H++){buf[H]=-1;buf1[H]=-1;}
...
if(size>1){MPI_Wait(&send1_request,&status1);
...
for(unsigned int p=0;p<jj;p++)
   {unsigned int aa=ok[p];
    v[aa]='1';}}
else{unsigned int a1=0;
     unsigned int a2=0;
     while(buf1[a1]>=0){unsigned int p=buf1[a1];v[p+(rig-1)*cols]='0';a1++;}
     while(buf[a2]>=0){unsigned int p=buf[a2];v[p]='1';a2++;}}


//copio prima e ultima riga
if(size==1){for(unsigned int h=0;h<cols;h++)
  {v[(rig-1)*cols+h]=v[h];}}
return T;
}
...
bool B=false;bool R=false;bool B1=false;bool R1=false;unsigned int cont=0;
 if(o==1||vec[o]%2==0){
             while(cont<n){
                    B1=blu(rank,size);++cont;if(cont==n)break;
