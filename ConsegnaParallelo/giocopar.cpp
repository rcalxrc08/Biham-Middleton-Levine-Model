...
using namespace std;
                                 ...
                                 //NOTE:per i commenti dettagliati si veda la versione sequenziale
const char rc='2';
const char bc='1';
const char cv='0';
...
for(unsigned int j=0;j<rig;++j){
   int prig=-1;
   if(numsurig[j]<redcar.size() && redcar[numsurig[j]]%cols==0) {prig=redcar[numsurig[j]];}
...
for(unsigned int j=0;j<cols;++j){
   int prig=-1;
   if(numcol[j]<blucar.size()&&blucar[numcol[j]]%rig==0){prig=blucar[numcol[j]];}
...
//classe che calcola il numero di righe e di colonne della matrice
