...
traffico::traffico(unsigned int r,unsigned int c,const char *filename_X)
{
	...
	while(i<redcar.size())
	{                                //riempio il vettore della distribuzione(cumulata) dei due vettori
...
{            
	...
	{                                    //ciclo sul numero di righe
		...
		{      //tengo traccia della macchina sulla prima colonna(se c'è)
			prig=redcar[numsurig[j]];
		}
		for(unsigned int i=numsurig[j];i<numsurig[j+1];++i)
		{            //ciclo sul vettore rosso(solo sulla parte corrispondente alla riga che sta venendo analizzata)
			if((redcar[i]+1)%cols!=0)
			{                                   //movimento se non è sull'ultima colonna la macchina rossa
				if(v[redcar[i]+1]==cv)
				{ 
					if(!Ftot)
					{
			...
			{                                                      //movimento se la macchina rossa è sull'ultima colonna
...
bool traffico::blu()
{                                                //lo spostamento delle blu è uguale a quello delle rosse  
	bool Ttot=false; 
	int prig=-1; 
	...
	for(unsigned int j=0;j<cols;++j)
	{                                 //nelle coordinate delle macchine rosse(per righe)
...
void traffico::scambi(unsigned int o,unsigned int n)
{
	...
	if(o==0||vec[o-1]%2==0)
	{                                  //se non sono all'inizio del gioco e se le iterazioni che ho fatto al passo prima sono pari allora
		while(cont<n)
		{                                //inizio con le blu finche non finisco il numeor di iterazioni da fare       
			B=blu();
			++cont;
			if(cont==n)
				break;                           
	...
	{                                             //altrimenti si inizia con le rosse e si cicla fino a quando le iterazioni non finiscono
		while(cont<n)
		{
			R=red();
			++cont;
